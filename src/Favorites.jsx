  ...
  useEffect(() => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      try {
        ...
        // Ensure the item looks like a movie object
        if (parsed && parsed.imdbID && parsed.Title) {
          fav.push(parsed);
        }
      } catch (error) {
        console.warn(`Skipping invalid localStorage entry for key "${key}":`, error.message);
      }
    }
    updateList(fav);
  }, []);
